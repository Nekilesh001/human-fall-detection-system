"""
Experiment K Phase K1: 187-D Spatial-Augmented YOLO Pose 1D TCN Benchmark Pipeline.
Model: K1 1D TCN (86,434 params)

Inputs:
- processed_data/Le2i_baseline/pose_estimator_features/yolo_pose_k1/ ((50, 187) float32)

Outputs:
- checkpoints/le2i_yolo_k1/fold_{1..4}_best.pth
- R&D/ML_Baseline/results/yolo_k1_spatial_benchmark_results.json
- R&D/ML_Baseline/results/yolo_k1_spatial_benchmark_results.csv
"""
...
class ModelK1_SpatialTCN(nn.Module):
    """K1: 1D TCN with 187-D Spatial Feature Input (86,434 params)"""
    def __init__(self, input_dim=187, num_channels=[64, 64], kernel_size=3, fc_dim=32, dropout_p=0.5):
        super().__init__()
        layers = []
...
def train_le2i_yolo_k1_spatial():
    print("=" * 70)
    print("EXPERIMENT K PHASE K1: 187-D SPATIAL TCN BENCHMARK")
    print("=" * 70)
    ...
    json_path = os.path.join(res_dir, "yolo_k1_spatial_benchmark_results.json")
    with open(json_path, "w") as f:
        json.dump(k1_benchmark_results, f, indent=2)

    csv_rows = []
    for r in fold_results:
        csv_rows.append({
            "model": "Model K1: 187-D Spatial TCN",
            "parameters": 86434,
            "fold": r["fold"],
    ...
    csv_path = os.path.join(res_dir, "yolo_k1_spatial_benchmark_results.csv")
    pd.DataFrame(csv_rows).to_csv(csv_path, index=False)

    print("\n" + "=" * 70)
    print("EXPERIMENT K PHASE K1 TRAINING COMPLETE — RESULTS SAVED")
    print("=" * 70)

if __name__ == "__main__":
    train_le2i_yolo_k1_spatial()
